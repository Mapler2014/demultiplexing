...
def paramsParse():
    ...
    parser.add_argument('--indexFile', help = "index file   samplename\ti7indexsequence\n", required = True)
...
def index(indexFile):
    ...
    for eachLine in open(indexFile):
        if eachLine.strip() == '':
            continue
        sample, i7Index = eachLine.strip().split()
...
def demultiplexing(sampleIndex, read1, read2, outdir, mismatch):
    ...
    for eachSample in sampleIndex:
        sampleIndex[eachSample]['Read1'] = gzip.open(os.path.join(outdir, eachSample + '_' + sampleIndex[eachSample]['i7Index'] + '_1.fq.gz'), mode = 'wb', compresslevel = 1)
        sampleIndex[eachSample]['Read2'] = gzip.open(os.path.join(outdir, eachSample + '_' + sampleIndex[eachSample]['i7Index'] + '_2.fq.gz'), mode = 'wb', compresslevel = 1)
